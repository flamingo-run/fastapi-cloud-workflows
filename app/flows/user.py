...
async def hash_password(ctx: Context, data: SignupRequest) -> UserDraft:
    # Do not actually hash; simulate
    return UserDraft(email=data.email, hashed_password=f"hashed:{data.password}")


# Map draft -> idp request using AssignStep (compiled to YAML assign)
...
# Call external identity provider via HttpStep
idp_call = HttpStep(
    name="idp-call",
...
async def persist_user(ctx: Context, data: IdentityRes) -> UserCreated:
    # Simulate persistence using external id as user id
