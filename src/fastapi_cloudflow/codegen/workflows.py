...
def workflow_to_yaml_dict(wf: Workflow, base_url_expr: str = 'sys.get_env("BASE_URL")') -> dict[str, Any]:
    ...
    def _with_required_headers(existing: dict[str, Any] | None, include_run_id: bool) -> dict[str, Any]:
    ...
    for idx, node in enumerate(wf.nodes):
        ...
        if isinstance(node, HttpStep):
            ...
            args: dict[str, Any] = {"url": _as_yaml_expr(node.url), "body": f"${{{payload_var}}}"}
            args["headers"] = _with_required_headers(node.headers or {}, have_run_id)
            if node.auth:
                args["auth"] = node.auth
            if node.timeout:
            ...
            if not have_run_id:
                # Capture run id from first response headers
                steps.append(
                    {
                        f"capture_run_id_{idx}": {
                            "assign": [{"run_id": f"${{{result_var}.headers['X-Workflow-Run-Id']}}"}]
                        }
                    }
                )
                have_run_id = True
        ...
        args = {
            "url": url_expr,
            "body": f"${{{payload_var}}}",
            "headers": _with_required_headers({}, have_run_id),
        }
...
def emit_workflow_yaml(wf: Workflow, out_dir: Path) -> Path:
